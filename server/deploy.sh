...
if [ -f /etc/apt/sources.list.d/caddy-stable.list ] || grep -r "cloudsmith.io.*caddy" /etc/apt/sources.list.d/ > /dev/null 2>&1; then
    ...
    echo -e "${GREEN}✅ 已移除有问题的仓库源${NC}"
else
    echo -e "${GREEN}✅ 仓库源检查通过${NC}"
fi

echo ""

# ==================== 3. 检测并安装系统依赖 ====================
echo -e "${GREEN}[3/9] 检测并安装系统依赖...${NC}"
...
# ==================== 5. 创建 Python 虚拟环境 ====================
echo -e "${GREEN}[5/9] 创建 Python 虚拟环境...${NC}"
...
EOF
    ...
    if grep -q "GEMINI_API_KEY=your_gemini_api_key_here" backend/.env || ! grep -q "GEMINI_API_KEY=" backend/.env; then
        ...
        echo -e "${GREEN}✅ 后端配置文件检查完成${NC}"
    fi
fi

# 修复前端配置（允许外部访问）
echo -e "${YELLOW}检查前端配置...${NC}"
if ! grep -q "host: '0.0.0.0'" frontend/vite.config.ts 2>/dev/null; then
    echo -e "${YELLOW}修复前端配置，允许外部访问...${NC}"
    # 在 server: { 后添加 host: '0.0.0.0',
    sed -i "/server: {/a\    host: '0.0.0.0'," frontend/vite.config.ts
    echo -e "${GREEN}✅ 前端配置已修复${NC}"
else
    echo -e "${GREEN}✅ 前端配置检查通过${NC}"
fi

# 修复后端CORS配置（允许所有来源）
echo -e "${YELLOW}检查后端CORS配置...${NC}"
if grep -q 'allow_origins=\["http://localhost:5173"' backend/app/main.py 2>/dev/null; then
    echo -e "${YELLOW}修复后端CORS配置，允许外部访问...${NC}"
    sed -i 's/allow_origins=\["http:\/\/localhost:5173", "http:\/\/localhost:3000"\]/allow_origins=["*"]/g' backend/app/main.py
    echo -e "${GREEN}✅ 后端CORS配置已修复${NC}"
else
    echo -e "${GREEN}✅ 后端CORS配置检查通过${NC}"
fi

echo ""

# ==================== 8. 安装前端依赖 ====================
echo -e "${GREEN}[8/9] 安装前端依赖...${NC}"
...
# ==================== 9. 检查端口占用和防火墙 ====================
echo -e "${GREEN}[9/9] 检查端口占用和防火墙...${NC}"
...
# 检查 5173 端口
if lsof -ti :5173 > /dev/null 2>&1 || netstat -tlnp 2>/dev/null | grep -q ":5173 "; then
...
if command -v ufw &> /dev/null; then
    ...
    if sudo ufw status | grep -q "Status: active"; then
        echo -e "${YELLOW}防火墙已启用，检查端口规则...${NC}"
        if ! sudo ufw status | grep -q "5173"; then
            echo -e "${YELLOW}添加防火墙规则...${NC}"
...
echo -e "   - 网络: ${GREEN}http://${SERVER_IP}:5173${NC}"
echo ""
echo -e "🔧 后端API:"
echo -e "   - 本地: ${GREEN}http://localhost:8000${NC}"
echo -e "   - 网络: ${GREEN}http://${SERVER_IP}:8000${NC}"
echo ""
echo -e "📚 API文档:"
...
echo ""
echo -e "🛑 停止服务:"
echo -e "   - 运行: ${YELLOW}./server/stop.sh${NC} 或 ${YELLOW}bash server/stop.sh${NC}"
echo ""
